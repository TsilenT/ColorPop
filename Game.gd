...
func perform_auto_match():
	# Call Helper
	var move = AutoMatcher.find_best_move(board_manager, level_manager, ROWS, COLS)
	var start = move.get("start", Vector2i(-1, -1))
	var end = move.get("end", Vector2i(-1, -1))
...
func spawn_floating_text(pos: Vector2, text: String, color: Color, txt_scale: float = 1.0, outline_color: Color = Color.BLACK):
	...
	ft.setup(text, color, txt_scale, outline_color)
